...
class StockServiceTest {
	...
	void 동시주문_재고관리_테스트() throws InterruptedException {
		...
		List<PromotionProductRequest> request = List.of(new PromotionProductRequest(promotionProduct.getId(), 1L));
		...
		for (int i = 0; i < threadCount; i++) {
			...
			executorService.execute(() -> {
				try {
					...
					stockService.decreaseStockAndCalculateTotalPrice(request);
