...
class LettuceLockStockFacadeTest {
	...
	private static final Logger log = LoggerFactory.getLogger(LettuceLockStockFacadeTest.class);
		...
		for (int i = 0; i < threadCount; i++) {
			...
			executorService.execute(() -> {
				try {
					...
					lettuceLockStockFacade.lockAndDecreaseStock(promotion.getId(), request);
